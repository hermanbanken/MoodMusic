...
import org.mp3transform.wav.WaveFileWriter;

public class MP3Tool extends Decoder {
    ...
    private static void convertAndHandle(InputStream sourceStream, IFrameHandler handler) throws IOException {
    	int frameCount = Integer.MAX_VALUE;
        MP3Tool decoder = new MP3Tool();
        ...
        try {
            for (int frame = 0; frame < frameCount; frame++) {
                ...
                handler.handleFrame(frame, stream);
                stream.closeFrame();
            }
        } finally {
            decoder.close();
    ...
    public void initOutputBuffer(int numberOfChannels, int freq, String fileName)
        ...
        outWave = new WaveFileWriter(fileName, freq, (short) 8,
