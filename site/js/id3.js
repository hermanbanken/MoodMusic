...
(function() {
	...
	function readFileData(file, callback) {
		...
		fr.onload = function(e){
			var data = new BinaryFile(e.target.result.substr(-128,128), 0, 128);
			var tags = readTagsFromData(data);
			files[file.name] = tags;
	...
	ID3.loadTags = function(file, cb) {
		if (!files[file.name]) {
			readFileData(file, cb);
		} else {
			if (cb) cb();
		}
	}

	ID3.getAllTags = function(uri) {
		if (!files[uri]) return null;

		var tags = {};
		for (var a in files[uri]) {
			if (files[uri].hasOwnProperty(a))
				tags[a] = files[uri][a];
		}
		return tags;
	}

	ID3.getTag = function(uri, tag) {
		if (!files[uri]) return null;

		return files[uri][tag];
	}
})();
